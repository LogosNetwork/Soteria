        ...
        >
          <b-input-group size="lg">
            <b-form-input
              ...
              required
              type="number"
              :placeholder="`${$t('amountIn')} ${$t('logos')}`"
            />
...
export default {
  ...
  computed: {
    ...
    isValidAmount () {
      ...
      return bigInt(this.tokenAccount.balance).minus(bigInt(this.$Utils.minimumFee))
  ...
  methods: {
    ...
    withdrawLogos () {
      if (this.isValidAmount && this.destinationAccount !== null) {
        if (bigInt(this.tokenAccount.balance)
          .greaterOrEquals(bigInt(this.amountInMinorUnit))) {
          for (const accountAddress in this.$Wallet.accounts) {
            if (this.tokenAccount.controllerPrivilege(accountAddress, 'withdraw_logos')) {
              this.$Wallet.accounts[accountAddress].createWithdrawLogosRequest({
